...
#include <vector>
#include <cassert>
#include "link_list_developer.hpp"
...
int main() {
    ...
    assert(linklist_to_string(listA) == "1.2 4.5 3.4 40.2234");

    listA.pop_back();
    listA.pop_back();
    assert(linklist_to_string(listA) == "1.2 4.5");

    listA.clear();
    assert(linklist_to_string(listA) == ""); 

    // new link list
    std::vector<int> integer_vector = {4, 3, 4, 5, 6, 2, 6, 7};
	data_structures::link_list<int> integer_list(integer_vector);
    assert(linklist_to_string(integer_list) == "4 3 4 5 6 2 6 7");

    integer_list.push_back_range({10, 11, 23, 4, 55, 100});
    assert(linklist_to_string(integer_list) == "4 3 4 5 6 2 6 7 10 11 23 4 55 100");

    // std::move()
    std::vector<int> integer_vector_B = {100, 200, 200, 200, 500, 600};
    integer_list.push_back_range(std::move(integer_vector_B));
    assert(linklist_to_string(integer_list) == "4 3 4 5 6 2 6 7 10 11 23 4 55 100 "
                                               "100 200 200 200 500 600");

    // unique require sorted list
    integer_list.unique();
    assert(linklist_to_string(integer_list) == "4 3 4 5 6 2 6 7 10 11 23 4 55 100 200 "
                                               "500 600");
    integer_list.sort();
    integer_list.unique();
    assert(linklist_to_string(integer_list) == "2 3 4 5 6 7 10 11 23 55 100 200 500 600");

    integer_list.push_front(100);
    integer_list.push_front(200);
    integer_list.push_front_range({4, 5, 6, 7});
    assert(linklist_to_string(integer_list) == "4 5 6 7 200 100 2 3 4 5 6 7 10 11 23 55 "
                                               "100 200 500 600");

    integer_list.pop_front();
    integer_list.pop_back();
    assert(linklist_to_string(integer_list) == "5 6 7 200 100 2 3 4 5 6 7 10 11 23 55 "
                                               "100 200 500");

    integer_list.pop_front();
    integer_list.pop_front();
    assert(linklist_to_string(integer_list) == "7 200 100 2 3 4 5 6 7 10 11 23 55 "
                                               "100 200 500");

    integer_list.erase(100);
    integer_list.erase(200);
    integer_list.erase(3);
    assert(linklist_to_string(integer_list) == "7 2 4 5 6 7 10 11 23 55 500");

    integer_list.sort();
    assert(linklist_to_string(integer_list) == "2 4 5 6 7 7 10 11 23 55 500");

    if (integer_list.search(200) != nullptr)
        std::abort();

    integer_list.clear();
    assert(linklist_to_string(integer_list) == "");
