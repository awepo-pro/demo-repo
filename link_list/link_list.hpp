...
namespace data_structures {
	...
	private:
		...
		static size_t count; // move to struct node
		...
		using node_ptr = std::shared_ptr<Node>;			// pointer type of the Node lise

		node_ptr first;					// pointer before the first element, like dummy node
		node_ptr last;					// pointer to the actual last element

		node_ptr find_previous(T_type find_elem);
		void destroy();
		std::pair<node_ptr, node_ptr> serveral_to_linklist(const std::vector<T_type> &serveral_elem);
		node_ptr merge(node_ptr l1, node_ptr l2);
		node_ptr sort(node_ptr head);		
	public:
	 ...
	 */
	template<typename T_type>
	size_t link_list<T_type>::count = 0;
